...
export class ScrollStore {
  ...
  private retrieveScrollPos(event: NavigationStart) {
    ...
    if (retrievedScrollPos === undefined) {
      console.log(
        'No saved position for ' + event.url + ' scroll to zero instead'
      );
      this.scrollToZero();
    } else {
      console.log(
        'Postion found for ' + event.url + ' scroll to ' + retrievedScrollPos
      );
...
@NgModule({
  providers: [SessionStorageService, ScrollStore]
})
