...
export class FileHolder {
  ...
  constructor(public src: string, public file: File, public uid: string) { }
}
...
export class ImageUploadComponent implements OnInit, OnChanges {
  ...
  @Output()
  uploadStateChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
  @Output()
  uploadFinished: EventEmitter<FileHolder> = new EventEmitter<FileHolder>();
  @Output()
  updateArray: EventEmitter<FileHolder[]> = new EventEmitter<FileHolder[]>();
  ...
  deleteAll() {
    ...
    this.updateArray.emit(this.files);
  }
  ...
  ngOnChanges(changes) {
    // console.log('Passou ngOnChanges', changes);
    if (
  ...
  private processUploadedFiles() {
    for (let i = 0; i < this.uploadedFiles.length; i++) {
      ...
      // console.log('processUploadedFiles', data);

      let fileBlob: Blob, file: File, fileUrl: string, uid: string;
      ...
      this.files.push(new FileHolder(fileUrl, file, uid));
      this.fileCounter++; ////Resolveu número maximo
    }
    // console.log('processUploadedFiles', this.uploadedFiles);
    this.updateArray.emit(this.files);
  }
    ...
    for (let i = 0; i < filesToUploadNum; i++) {
      ...
      reader.addEventListener(
        ...
        (event: any) => {
          ...
          this.updateArray.emit(this.files);
          // console.log('uploadFiles -> files', this.files);
        },
        false
      );
